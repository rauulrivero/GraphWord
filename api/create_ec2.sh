...
# Crear un key pair
aws ec2 create-key-pair --key-name mi-clave-ec2 --query "KeyMaterial" --output text --region "$REGION" > mi-clave-ec2.pem
chmod 400 mi-clave-ec2.pem

# Crear un Security Group y capturar su ID
SECURITY_GROUP_ID=$(aws ec2 create-security-group --group-name mi-grupo-seguridad --description "Permitir SSH y HTTP" --query "GroupId" --output text --region "$REGION")
...
aws ec2 run-instances --image-id ami-01816d07b1128cd2d --instance-type t2.micro --key-name mi-clave-ec2 --security-group-ids "$SECURITY_GROUP_ID" --region "$REGION"
