...
SECURITY_GROUP_ID=$(aws ec2 create-security-group --group-name gui-grupo-seguridad --description "Permitir SSH y HTTP" --query "GroupId" --output text --region "$REGION")
...
aws ec2 authorize-security-group-egress --group-id "$SECURITY_GROUP_ID" --protocol tcp --port 80 --cidr 0.0.0.0/0 --region "$REGION"

# Lanzar una instancia EC2 usando el Security Group creado
aws ec2 run-instances --image-id ami-01816d07b1128cd2d --instance-type t2.micro --key-name mi-clave-gui-ec2 --security-group-ids "$SECURITY_GROUP_ID" --region "$REGION"
